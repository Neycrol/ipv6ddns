...
11) 若**通过**提案：必须通知协调者（coordinator），给出主写手的落地要求与注意事项
    （如验证等级、风险点、兼容性策略等）。
...
13) 只有最终复审通过（2/3 或 3/3）后，**由你发布 PR**（使用 pr-submit 技能）。
    PR 发布步骤（必须明确写出你将执行这些步骤）：
    - 确认当前分支为 iflow/<category>-<n>，且不在 main/master。
    - 确认 glm-lead 已完成提交并推送远端。
    - 使用 pr-submit skill 发布 PR（或明确等价步骤：gh pr create + gh pr comment）。
    - 发布后回报 PR 链接与简要风险说明。
