...
### Scenario 8: Netlink socket fails

**Symptoms:**
- "Netlink socket failed" in logs
- Daemon falls back to polling mode
...
**Solutions:**
- Ensure the daemon runs with appropriate permissions
- Polling mode works but uses more CPU
- The daemon automatically falls back to polling if netlink is unavailable

### Scenario 9: Permission denied errors

**Symptoms:**
- Permission errors in logs
- Netlink socket creation fails

**Possible causes:**
- The daemon needs CAP_NET_RAW capability for netlink
- Systemd service not configured correctly

**Solutions:**
```bash
# Ensure the systemd service is correctly installed
sudo systemctl daemon-reload
sudo systemctl enable --now ipv6ddns
```
